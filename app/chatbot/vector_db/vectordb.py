...
class VectorDB:
    ...
    def add_document(
        self, document: Union[str, List[str]], metadata: Union[Dict, List[Dict]]
    ):
        ...
        if len(document) != len(metadata):
            raise ValueError(
                "The number of documents must match the number of metadata entries."
            )
        ...
        current_id = (
            len(self.collection.get()["ids"]) if self.collection.get()["ids"] else 0
        )
        ids = [str(current_id + i) for i in range(len(document))]

        self.collection.add(
            documents=document, metadatas=metadata, ids=ids, embeddings=embeddings
        )
    ...
    def add_langchain_documents(self, documents: List[Document]):
        ...
        self.add_document(document=docs, metadata=metadatas)
